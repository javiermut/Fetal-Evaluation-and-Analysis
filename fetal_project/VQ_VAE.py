...
studies_pd = studies_pd[(studies_pd['tag_ga'] >= 41) & (studies_pd['tag_ga'] <= 42)]
print(studies_pd)
...
early_stopping = keras.callbacks.EarlyStopping(monitor='loss',patience=10,verbose=1,restore_best_weights=True)
...
np.random.seed(42)
idx = np.random.choice(len(x_test_scaled), 10, replace=False)
